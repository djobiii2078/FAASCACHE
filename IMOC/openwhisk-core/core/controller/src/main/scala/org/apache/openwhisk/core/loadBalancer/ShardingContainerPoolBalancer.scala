...
import ramcloudWrapper.Wrapper 
...
object ShardingContainerPoolBalancer extends LoadBalancerProvider {
   ...
   *
   * @param parameters the list of parameters given to the action
  ...
  def schedule(
    ...
    val wrapperRc: Wrapper = new Wrapper()

    if (numInvokers > 0) {

      if(fqn.asString.equals(loadConfigOrThrow[String]("ramcloud.migrator.name"))){
            //Good, now retrieve the object name to retrieve the host and find the corresponding invoker
            
            val rc_object_name = parameters.get("object_name") match {
              case None => ""
              case Some(x) => x.toString() 
            }

            logging.warn(this,s"action caught is a migrator, so we need to catch locator for ${rc_object_name}")
            val hostLookUp = wrapperRc.wrapperGetLocator(loadConfigOrThrow[String]("ramcloud.locator.url"),loadConfigOrThrow[String]("ramcloud.tableName.name"),rc_object_name)
            logging.warn(this,s"Locator for ${rc_object_name} is ${hostLookUp}. Checking for corresponding invoker")

              //Now get the invoker with the hostLookUp.
            val possibleInvoker = invokers.filter(_.status.isUsable).filter(_.id.rcHostIP.getOrElse("") == hostLookUp)
            if(possibleInvoker.nonEmpty) 
              {

                logging.warn(this,s"Invoker ${possibleInvoker(0).id.toInt} chosen for the migrator. Fire !!!")
                Some(possibleInvoker(0).id,true) 
              
              }else{

                logging.warn(this,s"No suitable invoker found, hope next try will be okay")
                None 
              } 
      ...
      if (invoker.status.isUsable && dispatched(invoker.id.toInt).tryAcquireConcurrent(fqn, maxConcurrent, slots)) {
        ...
        if (stepsDone == numInvokers + 1) {
          ...
          schedule( parameters, maxConcurrent, fqn, invokers, dispatched, slots, newIndex, step, stepsDone + 1)
        }
      }
    } else {
      None
