...
abstract class CommonLoadBalancer(config: WhiskConfig,
  ...
  protected[loadBalancer] val payloads = TrieMap[ActivationId, ActivationMessage]() 
  protected[loadBalancer] val actionsTrieMap = TrieMap[ActivationId, WhiskActionMetaData]()
  ...
  protected def setupActivation(msg: ActivationMessage,
    ...
    payloads.getOrElseUpdate(msg.activationId,msg)
    actionsTrieMap.getOrElseUpdate(msg.activationId,action.toWhiskAction)
  ...
  protected def processResult(aid: ActivationId,
    ...
    //We need to get the result and obtain the output to feed the model 
    //if needed 
    if(!memSlimmer.canPredict(actionsTrieMap.getOrElse(aid,null))){
        response match {
          case Left(x) => {
                logging.info(this, s"adding new data to dataset for action ${activationSlots.getOrElse(aid,null).fullyQualifiedEntityName}")(tid)
          }
          
          case Right(x) => {
                memSlimmer.slim(actionsTrieMap.getOrElse(aid,null),payloads.getOrElse(aid,null).content,Some((x.resultAsJson.fields("memory")).toString()))(logging)
