...
import org.apache.openwhisk.core.controller.ml.FinalMemSlimmer
...
trait WhiskActionsApi extends WhiskCollectionAPI with PostActionActivation with ReferencedEntities {
  ...
  protected val memSlimmer: FinalMemSlimmer = FinalMemSlimmer()

  /** RAMcloud wrapper to trigger scaling up and down operations **/

  protected val rcWrapperUtil: Wrapper  = Wrapper()
                ...
                case Success(_) =>
                  ...
                  if (allowInvoke) {
                    ...
                    if(memSlimmer.canPredict(actionWithMergedParams)){
                      ...
                      val predictedMem = memSlimmer.slim(actionWithMergedParams,payload,None)(logging)
                      val toPlugInMemory = actionWithMergedParams.limits.memory.megabytes - predictedMem

                      //Routine to plug the exceeding memory in the cache 
                      rcWrapperUtil.wrapperScaleUp("","",true,"") //rcScalaClient.scaleUp(toPlugInMemory)
                      
                      val tmp = actionWithMergedParams.limits.copy()
                      val lims = ActionLimits(
                                                            tmp.timeout,
                                                            MemoryLimit(predictedMem.toString()),
                                                            tmp.logs,
                                                            tmp.concurrency
                                                          )
                      
                      doInvoke(user,actionWithMergedParams.copy(limits=lims),payload,blocking,waitOverride,result)
    ...
    onComplete(invokeAction(user, actionWithMergedParams, payload, waitForResponse, cause = None)) {
      case Success(Left(activationId)) =>
        ...
        respondWithActivationIdHeader(activationId) {
          ...
          // val backgroundPoller = Future {
          //   pollActivation(docid, UserContext(user), result, activationId, 300.second, )
          // }
          // logging.info(this,s"Need to poll for activation result to feed the model for ${actionWithMergedParams.name.toString}")
        }
      ...
      case Success(Right(activation)) =>
        ...
        respondWithActivationIdHeader(activation.activationId) {
          if (activation.response.isSuccess) {
            ...
            if(feed){
              logging.info(this,s"Adding new entry for ${actionWithMergedParams.name.toString} in .arff")
              memSlimmer.slim(actionWithMergedParams,payload,Some(response.fields("memory").convertTo[String]))(logging)
            }
