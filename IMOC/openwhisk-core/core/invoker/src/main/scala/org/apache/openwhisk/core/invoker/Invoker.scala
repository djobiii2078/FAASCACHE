...
//Adding rcHostIP as parameter to the commandLine Argument for invoker creation to pass it to the controller 
//Needs to confirm status.hostIP work 

case class CmdLineArgs(uniqueName: Option[String] = None, id: Option[Int] = None, displayedName: Option[String] = None, rcHostIP: Option[String] = None)
  ...
  def main(args: Array[String]): Unit = {
    ...
    def parse(ls: List[String], c: CmdLineArgs): CmdLineArgs = {
      ls match {
        ...
        case "--rcHostIP" :: rcHostIP :: tail =>
          parse(tail, c.copy(rcHostIP = nonEmptyString(rcHostIP)))
        case Nil => c
    ...
    //Adding rcHostIP to the invokerInstanceId passed down
    val invokerInstance =
      InvokerInstanceId(assignedInvokerId, cmdLineArgs.uniqueName, cmdLineArgs.displayedName, cmdLineArgs.rcHostIP, poolConfig.userMemory)
