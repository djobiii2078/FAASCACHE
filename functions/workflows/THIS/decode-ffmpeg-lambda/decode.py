...
def prepareffmpeg(url):

    _authurl = "http://"+url+":8080/auth/v1.0"
    conn = Connection(
        authurl=_authurl,
        user=_user,
        key=_key,
    ...
    _, ffmpegExec = conn.get_object(container_name, "ffmpeg")
    with open(FFMPEG_PATH, 'w' ) as local:
        local.write(ffmpegExec)
...
def upload_output_to_s3(bucketName, filePrefix, url):
   ...
   # s3 = boto3.client('s3', config=botocore.client.Config(
    ...
    def upload_file(localFilePath, uploadFileName, fileSize):
        ...
        try:
            ...
            with open(localFilePath, 'rb') as ifs:
                ...
                conn.put_object(bucketName, uploadFileName, contents=ifs) #, content_type="image/jpeg")
    ...
    print 'Uploaded %d files to Swift [total=%dKB]' % (count, totalSize >> 10)
...
def handler(event, context):
    ...
    _authurl = "http://"+event['url']+":8080/auth/v1.0"
    conn = Connection(
        authurl=_authurl,
        user=_user,
        key=_key,
    ...
    _, videostream = conn.get_object(container_name, videoUrl)
    
    with open(INPUT_FILE_PATH,'w') as local:
        local.write(videostream)
	...
	start_load = now()
        if outputBucket:
            fileCount, totalSize = upload_output_to_s3(
                outputBucket, outputPrefix, event['url'])
        else:
